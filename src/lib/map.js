...
export async function map(node) {
  ...
  await loadModules(
    [
      ...
      .concat(expandList),
    esriLoaderOptions
  ).then(([EsriMap, MapView, Expand, ...widgetArray]) => {
    const params = new URLSearchParams(window.location.search.toLowerCase());
    const webmapId = params.get("webmapid");
    if (webmapId) {
      ...
      console.error(
        `No webmapid found in url search params - please add ${
          Array.from(params).length > 0 ? "&" : "?"
        }webmapid=<id> to the url`
      );
    }
  });
